...
static struct class*  dog_oss_class  = NULL;
static struct device* dog_oss_device = NULL;
...
//
static int __init dog_oss_char_init(void)
{
   ...
   // 
   dog_oss_class = class_create(THIS_MODULE, CLASS_NAME);

   if( IS_ERR(dog_oss_class) )
   { 
   ...
   //
   dog_oss_device = device_create(dog_oss_class, NULL, MKDEV(g_majornum, 0), NULL, DEVICE_NAME);

   if( IS_ERR(dog_oss_device) )
   {
      class_destroy(dog_oss_class);

      unregister_chrdev(g_majornum, DEVICE_NAME);

      printk("[-] Failed to create device class\n");

      return PTR_ERR(dog_oss_device);
   }
...
//
static void __exit dog_oss_char_exit(void)
{
   ...
   //
   device_destroy(dog_oss_class, MKDEV(g_majornum, 0));     

   //
   // unregister the class
   //
   class_unregister(dog_oss_class);                          
...
module_init(dog_oss_char_init);
module_exit(dog_oss_char_exit);
